...
### 有`$= dv.pages('"02-Reading/4-betternotes" AND #unread').where(p=>p.file.name[0]=='@').file.length`篇未读文献

```dataview
TABLE file.cday AS Date
FROM "02-Reading/4-betternotes" AND #unread
...
```tagcloud
source: query
query: "02-Reading/4-betternotes"
minFontSize: 8
weight: 3
shuffle: false
```
...
### 组会已分享`$= dv.pages('"02-Reading/4-betternotes"').where(p=>p.share).file.length`篇
